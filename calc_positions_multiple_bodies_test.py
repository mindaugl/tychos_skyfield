...
class PlanetObj:
    ...
    def move_planet(self, pos):
        ...
        NOTE: only can use function once, as everytime it modifies values
        :param pos: float
            Position in degrees to rotate around y-axis
        :return:
        """
        self.rotation = self.rotation * R.from_euler('y', self.speed * pos - self.start_pos,
                                                     degrees = True)
    ...
    def move_planet_basic(self, pos):
        """
        Moves planet by specified pos, assuming speed = 0 and start_pos = 0.
